...
def create_single_screen(panels: dict, labels: dict, screen_w: int = 1600, screen_h: int = 900) -> np.ndarray:
    ...
    layout = {
        ...
        'bottom_left': ('result', (panel_h, 0)),
        'bottom_right': ('channels', (panel_h, panel_w)),
    }
    ...
    cv2.putText(screen, "Click panel to fullscreen | 'q' quit | 's' save", 
...
def mouse_callback(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if y < panel_h and x < panel_w:
            ...
            clicked = 'channels'
        # Use param dict to store state instead of globals
